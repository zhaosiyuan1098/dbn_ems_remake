...
option = Option()
class RBM:
    def __init__(self, n_visible, n_hidden, Option:option):
        self.n_visible =n_visible
        self.n_hidden =n_hidden
        self.mode = option.rbm_mode  # bernoulli or gaussian RBM
        self.lr = option.rbm_lr  # Learning rate for the CD algorithm
        self.epochs = option.rbm_epochs # Number of iterations to run the algorithm for
        self.batch_size = option.rbm_batch_size
        self.k = option.rbm_k
        self.optimizer = option.rbm_optimizer
        self.beta_1 = option.rbm_beta_1
        self.beta_2 = option.rbm_beta_2
        self.epsilon = option.rbm_epsilon
        self.savefile = option.rbm_savefile
        self.early_stopping_patience = option.rbm_early_stopping_patience
        self.stagnation = option.rbm_stagnation
        self.previous_loss_before_stagnation = option.rbm_previous_loss_before_stagnation
        self.progress = option.rbm_progress
        self.m = [0, 0, 0]
        self.v = [0, 0, 0]
        self.m_batches = {0: [], 1: [], 2: []}
        self.v_batches = {0: [], 1: [], 2: []}

        if torch.cuda.is_available() and option.rbm_gpu == True:
            dev = "cuda:0"
        else:
            dev = "cpu"
